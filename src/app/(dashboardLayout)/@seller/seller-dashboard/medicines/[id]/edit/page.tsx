...
import { ArrowLeft, Loader2, Save, Pill, DollarSign, Package, Factory, List, Terminal, Activity, ShieldCheck, Sparkles, FileText, CheckCircle2, X } from "lucide-react";
...
const medicineSchema = z.object({
    ...
    price: z.number().positive("Valuation must be positive"),
...
export default function EditMedicinePage() {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            setIsSaving(true);
            const toastId = toast.loading("Updating registration protocol...");
            const res = await medicineService.updateMedicine(id, value);

            if (!res.error) {
                toast.success("Asset manifest updated successfully", { id: toastId });
                router.push("/seller-dashboard/medicines");
            } else {
                toast.error(res.error || "Update protocol failed", { id: toastId });
                setIsSaving(false);
            }
        },
    });
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            console.log(">>> [EDIT_PAGE] Initializing data fetch for ID:", id);
            ...
            if (!medRes.error && medRes.data) {
                ...
                // Explicitly set each field value to ensure TanStack Form state is updated correctly
                ...
                form.setFieldValue("stock", medRes.data.stock || 0);
                ...
                console.error(">>> [EDIT_PAGE] Failed to load medicine data:", medRes.error);
                toast.error("Failed to load medicine data. Asset may be offline.");
                router.push("/seller-dashboard/medicines");
            }
    ...
    if (isLoading) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground animate-pulse">Decrypting Asset Metadata...</p>
            </div>
        );
    }

    return (
        <div className="max-w-5xl mx-auto space-y-12 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center justify-between">
                <Button variant="ghost" asChild className="group text-gray-400 hover:text-primary transition-all rounded-xl px-4 hover:bg-primary/5">
                    <Link href="/seller-dashboard/medicines" className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em]">Abort Edit Protocol</span>
                    </Link>
                </Button>

                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-600 uppercase tracking-widest text-right">
                    <span>Ledger</span>
                    <div className="size-1 rounded-full bg-gray-800" />
                    <span className="text-primary/70">Modification Protocol</span>
                    <div className="size-1 rounded-full bg-gray-800" />
                    <span className="text-gray-500 font-mono">{id.slice(-8).toUpperCase()}</span>
                </div>
            </div>
            ...
            <div className="space-y-4">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/10 border border-primary/20 rounded-full text-primary text-[10px] font-black uppercase tracking-[0.2em]">
                    ...
                    <span>System Node: Active Uplink</span>
                </div>
                <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter uppercase leading-none">
                    Asset <span className="text-primary italic">Modification</span>
                </h1>
                <p className="text-sm text-muted-foreground font-medium max-w-xl">
                    Updating technical directives and deployment parameters for asset signature: <span className="text-white font-bold">{id}</span>
                </p>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <CardDescription className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] mt-1">Re-evaluating Logistics and Directives</CardDescription>
                            </div>
                        </div>
                                    ...
                                    children={(field) => (
                                        <div className="space-y-3 group">
                                            <Label htmlFor="stock" className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 group-focus-within:text-primary transition-colors flex items-center gap-2">
                                                <Package className="size-3" /> Current Deployment Units
                                            </Label>
                                            <div className="relative">
                                                <div className="absolute -inset-1 bg-primary/10 rounded-xl blur opacity-0 group-focus-within:opacity-100 transition-opacity" />
                                                <Input
